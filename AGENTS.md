...
- **同一个概念在不同 booru 的标签词表里可能根本不存在**：`scenery` 在 yande.re 没有、
  在 safebooru 有 64812 张。接口对不存在的标签只回「200 + 空数组」，**跟"真没搜到"长得
  一模一样**——所以 `yandere`/`safebooru` 现在在 `search` 里**先逐个校验标签**
  （每标签一次精确查，实测 0.1KB / 0.5s；别打全量标签表的主意，yande.re 全量 9.8MB、
  safebooru 没有全量接口）。加同类图源时照做，别让调用方对着静默的 0 条猜。
- **"校验失败"必须区别于"标签不存在"**：网络挂了、响应不是预期格式时只能跳过校验按原样搜，
  绝不能判成"标签不存在"——否则对方一改接口，所有合法查询都会被我们自己的校验判死。
  safebooru 的 tag 接口只给 XML，所以 `parse_tag_xml` 把"空 `<tags>`"（真不存在）和
  "不是 XML"（读不到答案）分开处理：前者返回空列表，后者抛 `ProviderError`。
- **标签校验是"查询"的属性，不是"页"的属性**：只在 `cursor is None` 时做，翻页不重复查。

## 待办

见 [TODO.md](TODO.md)：已知缺口、**验证过的接口形态**和验收标准都在那里。
动手做某条之前先把它读一遍，别重新摸一遍接口。
